...
#include "impl/settings_game_card_impl.hpp"
...
namespace ams::settings::factory {
    ...
    Result GetGameCardKey(GameCardKey *out) {
        ...
        R_TRY_CATCH(settings::impl::GetGameCardKey(out)) {
    ...
    Result GetGameCardCertificate(GameCardCertificate *out) {
        /* Non calibration data errors represent fundamental system failure. */
        R_TRY_CATCH(settings::impl::GetGameCardCertificate(out)) {
            R_CATCH_RETHROW(settings::factory::ResultCalibrationDataFileSystemCorrupted)
            R_CATCH_RETHROW(settings::factory::ResultCalibrationDataShaError)
        } R_END_TRY_CATCH_WITH_ABORT_UNLESS;
        return ResultSuccess();
    }
